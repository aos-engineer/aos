...
export function generateMemoryYaml(
  projectName: string,
  provider: "expertise" | "mempalace" | "graphify",
): string {
  const providerBlock =
    provider === "mempalace" ? "mempalace" : provider === "graphify" ? "graphify" : "expertise";
  return `# AOS Memory Configuration
# Generated by: aos init
api_version: aos/memory/v1

# Memory provider: "mempalace" | "graphify" | "expertise" (built-in fallback)
...
# Graphify settings (used when provider: graphify)
# Knowledge-graph recall. Install with: uv tool install "graphifyy[mcp]"
# then build a graph: graphify build .aos/graphify/corpus
# graphify:
#   corpus_path: .aos/graphify/corpus
#   graph_path: graphify-out/graph.json
#   mcp_command: graphify-mcp
#   build_command: graphify
#   auto_build: false        # set true to rebuild on every remember (LLM cost)
#   max_wake_tokens: 1200
